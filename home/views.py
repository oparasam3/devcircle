    ...
    if request.method == 'POST':
        ...
        if cart:
            ...
            if basket:
                ...
                basket.save()
                messages.success(request, format_html("one item added ... <a href='http://18.224.202.128/cart'>view cart</a>"))
                return redirect('home')
            else:
                newitem = Cart()
...
def pay(request):
    if request.method == 'POST':
        ...
        cburl = 'http://18.224.202.128/callback' #payment confirmation page 
